...
def fixture_id_resource_positive(request):
    return request.param


@pytest.fixture(params=[-1, 0, 201, "string", False, [1, 2, 3], None])
def fixture_id_resource_negative(request):
    return request.param
...
def fixture_payload_resource_positive(request):
    return request.param


@pytest.fixture(params=[1, 5, 10])
def fixture_test_filtering_positive_by_userid(request):
    return request.param


@pytest.fixture(params=[True, False])
def fixture_test_filtering_positive_by_completed(request):
    return request.param
...
@pytest.fixture(params=["et praesentium aliquam est",
                        ...
                        "delectus aut autem"])
def fixture_test_filtering_positive_by_title(request):
    return request.param
